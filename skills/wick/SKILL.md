...
2. `wick read` — structured body when the excerpt is not enough. Prefer this over `open`.
3. `wick plan` / `wick ask --q terms` — suggestions or a filter. Same ~8s observe cache.
...
| Prefer snap, then `read` | Dump `open` or reach for `cu` unless canvas / widget / challenge |
