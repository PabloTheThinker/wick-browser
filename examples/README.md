...
- `search.json` — headless `search` then `snap` of a result (no screenshots).
...
```bash
wick search "example domain"                           # headless SERP → results[].url
...
- **Observe:** `search`, `open`, `fetch`, `probe`, `tree`, `links`, `snap`
